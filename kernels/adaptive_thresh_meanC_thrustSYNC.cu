...
__global__ void adaptive_thresh_thrustSYNC_Kernel(int *row_summed_pixels, unsigned char *Pixel_in, unsigned char *Pixel_out,
                                       int width, int height, int maxValue, int filterSize, int constance) {
...
torch::Tensor adaptive_thresh_meanC_thrustSYNC(torch::Tensor img, int filterSize, int constance, int maxValue) {
    ...
    adaptive_thresh_thrustSYNC_Kernel<<<dimGrid, dimBlock, shared_memorySize>>>(
